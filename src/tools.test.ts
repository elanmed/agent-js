...
describe("tools", () => {
  ...
  describe("executeCreateFileTool", () => {
    it("creates a new file and returns success", () => {
      ...
      assert.equal(result.type, "tool_result");
      assert.ok(!result.is_error);
      assert.ok(result.content.includes("created successfully"));
  ...
  describe("executeViewFileTool", () => {
    it("returns file contents with line numbers", () => {
      ...
      assert.equal(result.type, "tool_result");
      assert.ok(!result.is_error);
      assert.ok(result.content.includes("1\taaa"));
      assert.ok(result.content.includes("2\tbbb"));
      assert.ok(result.content.includes("3\tccc"));
    });
    ...
    it("treats end_line=-1 as end of file", () => {
      ...
      assert.ok(result.content.includes("3\tc"));
    ...
    it("lists directory contents for a directory path", () => {
      ...
      const result = executeViewFileTool(call);
      assert.ok(!result.is_error);
      assert.ok(result.content.includes("alpha.txt"));
      assert.ok(result.content.includes("beta.txt"));
    });
  ...
  describe("executeStrReplaceTool", () => {
    it("replaces old_str with new_str when exactly one match exists", () => {
      ...
      assert.ok(!result.is_error);
      assert.ok(result.content.includes("updated successfully"));
    ...
    it("returns is_error when old_str matches more than once", () => {
      ...
      const result = executeStrReplaceTool(call);
      assert.equal(result.is_error, true);
      assert.ok(result.content.includes("2 times"));
    });
  ...
  describe("getToolResultBlock", () => {
    it("handles the bash tool", async () => {
      const call = makeToolCall({ input: { command: "echo test" } });
      const result = await getToolResultBlock(call);
      assert.equal(result.type, "tool_result");
      assert.equal(result.tool_use_id, "tool_1");
    });
    ...
    it("throws for an unknown tool name", async () => {
      const call = makeToolCall({ name: "unknown_tool" });
      await assert.rejects(
        () => getToolResultBlock(call),
